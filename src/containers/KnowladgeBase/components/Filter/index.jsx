    ...
    return <aside className={styles.filter_container}>
        ...
        <ul className={styles.filter_list}>
            {filterConstants.map(filterConstant => {
                ...
                const isActive = filterOptions.find(filterOption => filterOption.min === filterConstant.min)

                return <li  key={filterConstant.min} className={cx(styles.filter_list_item,isActive && styles.active)}>
                    <button className={cx(styles.filterBtn)} onClick={()=>handleFilterClick(filterConstant)}>
