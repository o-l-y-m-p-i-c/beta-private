...
export const KnowledgeBaseContainer = () => {
    ...
    const handleFilterClick = (option = {
        ...
        if(foundFilterOption){
            ...
            navigate(`/knowledge/${1}?options=${formatGetOptions([...filterOptions,option])}`);
        }
    ...
    const handleNavigation = (direction) => {
        ...
        if(filterOptions.length === 0){
           navigate(`/knowledge/${newPageID}`);
            ...
            navigate(`/knowledge/${newPageID}?options=${formatGetOptions(filterOptions)}`);
        }
    };
    ...
    useEffect(() => {
        ...
        const newArr = filterConstants.filter(filterConstant => {
            if(options && options !== ""){
                const isFound = options.split(',').find(option => option === filterConstant.title)
                // console.log(isFound)
                if(isFound){
                    return true
                }
